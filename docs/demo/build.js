...
function findPlaywright() {
	...
	for (const p of candidates) {
		try {
			return { mod: require(p), dir: path.dirname(require.resolve(`${p}/package.json`)) };
...
function setup() {
	if (!has('ffmpeg') || !has('ffprobe')) {
		throw new Error('ffmpeg is missing and cannot be installed for you. Run: brew install ffmpeg');
	}
	...
	if (pw) {
		...
		console.log(`installing playwright ${PLAYWRIGHT_VERSION} into docs/demo/.cache …`);
		fs.mkdirSync(CACHE, { recursive: true });
		run('npm', ['install', '--prefix', CACHE, '--no-save', '--no-audit', '--no-fund',
			`playwright@${PLAYWRIGHT_VERSION}`]);
		pw = findPlaywright();
		if (!pw) throw new Error('npm reported success but playwright still does not resolve');
	}
...
const durationMs = (f) => Math.round(parseFloat(
...
function measureHead(webm) {
	const r = spawnSync('ffmpeg',
		['-hide_banner', '-nostats', '-i', webm, '-vf', 'blackdetect=d=0.10:pix_th=0.02:pic_th=0.98', '-f', 'null', '-'],
		{ encoding: 'utf8' });
	for (const seg of String(r.stderr || '').match(/black_start:([\d.]+)\s+black_end:([\d.]+)/g) || []) {
...
async function main() {
	...
	if (!pw) throw new Error('playwright not found. Run once: npm run demo:setup');
	...
	try {
		...
		throw new Error(`chromium failed to launch. Run once: npm run demo:setup\n\n  ${e.message.split('\n')[0]}`);
	}
	const ctx = await browser.newContext({
		viewport: { width: W, height: H },
		deviceScaleFactor: 1,
		...(shotsOnly ? {} : { recordVideo: { dir: OUT, size: { width: W, height: H } } }),
	});
	const page = await ctx.newPage();
	let pageErr = null;
	page.on('pageerror', (e) => { pageErr = e.message; });
	await page.addInitScript((slow) => { window.__SLOW = slow; }, shotsOnly ? 1 : SLOW);
	...
	if (shotsOnly) {
		...
		await page.evaluate(() => { window.__shotsMode = true; });
	...
	const webm = path.join(OUT, fs.readdirSync(OUT).filter((f) => f.endsWith('.webm')).pop());
	const headMs = measureHead(webm);
	const raw = durationMs(webm);
	const mp4 = path.join(OUT, 'transcripta-demo.mp4');
	console.log(`  recording ${(raw / 1000).toFixed(1)} s, black head ${headMs} ms`);

	execFileSync('ffmpeg', [
		'-y', '-hide_banner', '-loglevel', 'error',
		'-ss', (headMs / 1000).toFixed(3), '-i', webm,
		'-vf', `setpts=PTS/${SLOW},fps=30,scale=${W}:${H}:flags=lanczos`,
