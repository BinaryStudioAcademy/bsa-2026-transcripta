...
- **No logo exists.** The wordmark is plain Fraunces 600 "Transcripta"; the app lockup is a 26px seal-red tile with a Fraunces "T" beside the wordmark (see `guidelines/wordmark.html`). Do not draw a logo.
...
- `templates/` — copyable layout shells: `app-shell/` (sidebar + topbar), `focus-shell/` (verification, no sidebar), `auth-card/` (centered card).
